...
seturl() {
	ALPINE_VER=$(curl -s http://dl-cdn.alpinelinux.org/alpine/v3.14/releases/$SETARCH/latest-releases.yaml | grep -m 1 -o version.* | sed -e 's/[^0-9.]*//g' -e 's/-$//')
	if [ -z "$ALPINE_VER" ] ; then
		exit 1
	fi
	ALPINE_URL="http://dl-cdn.alpinelinux.org/alpine/v3.14/releases/$SETARCH/alpine-minirootfs-$ALPINE_VER-$SETARCH.tar.gz"
}
...
finalwork() {
	[ ! -e ${DESTINATION}/finaltouchup.sh ] && curl --silent -LO https://raw.githubusercontent.com/pebrianz/termux-alpine-v3.14/main/finaltouchup.sh
	if [ "${MOTD}" = "ON" ]; then
